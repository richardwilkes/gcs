 ...
 */
public class TKRowIterator<T extends TKRow> implements Iterator<T>, Iterable<T> {
	private List<TKRow>			mList;
	private int					mIndex;
	private TKRowIterator<T>	mIterator;
	 ...
	 */
	public TKRowIterator(TKOutlineModel model) {
		this(model.getTopLevelRows());
	}

	private TKRowIterator(List<TKRow> rows) {
		mList = rows;
	}
	...
	@SuppressWarnings("unchecked") public T next() {
		if (hasNext()) {
			if (mIterator == null) {
				TKRow row = mList.get(mIndex++);

				if (row.hasChildren()) {
					mIterator = new TKRowIterator<T>(row.getChildren());
				}
				return (T) row;
			}
