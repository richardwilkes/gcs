...
/** Represents a single row of data within an {@link TKOutlineModel}. */
public abstract class TKRow {
	private TKOutlineModel		mOwner;
	private int					mHeight;
	private boolean				mOpen;
	private TKRow				mParent;
	/** The children of this row. */
	protected ArrayList<TKRow>	mChildren;
	 ...
	 */
	void applyUndoSnapshot(TKOutlineModel owner, TKRowUndoSnapshot snapshot) {
		mOwner = owner;
		mParent = snapshot.getParent();
		mOpen = snapshot.isOpen();
		if (canHaveChildren()) {
			mChildren.clear();
			for (TKRow child : snapshot.getChildren()) {
				mChildren.add(child);
				child.mParent = this;
			}
		}
	...
	/** @param owner The owning model. */
	void applyUndoSnapshot(TKOutlineModel owner) {
		mOwner = owner;
		mParent = null;
	}
	 ...
	 */
	public BufferedImage getIcon(@SuppressWarnings("unused") TKColumn column) {
		return null;
	}
	 ...
	 */
	public abstract Object getData(TKColumn column);

	/**
	 * @param column The column.
	 * @return The data for the specified column as text.
	 */
	public abstract String getDataAsText(TKColumn column);
	 ...
	 */
	public abstract void setData(TKColumn column, Object data);
	 ...
	 */
	public int getPreferredHeight(List<TKColumn> columns) {
	...
	public TKOutlineModel getOwner() {
		return mOwner;
	}
	 ...
	 */
	void setOwner(TKOutlineModel owner) {
		mOwner = owner;
	}
	 ...
	 */
	public TKRow getChild(int index) {
	 ...
	 */
	public int getIndexOfChild(TKRow row) {
		if (canHaveChildren()) {
	 ...
	 */
	public boolean insertChild(int index, TKRow row) {
		if (canHaveChildren()) {
			int max;
	 ...
	 * @return <code>true</code> if the row was added, <code>false</code> if it was not.
	 */
	public boolean addChild(TKRow row) {
		if (canHaveChildren()) {
			row.removeFromParent();
			mChildren.add(row);
			row.mParent = this;
	 ...
	 *         of this row.
	 */
	public boolean removeChild(TKRow row) {
	 ...
	 */
	public boolean isChildOf(TKRow parent) {
	 ...
	 */
	public boolean isDescendentOf(TKRow row) {
		TKRow parent = mParent;
	...
	public TKRow[] getPath() {
		...
		return list.toArray(new TKRow[0]);
	}
