...
/** A timer task that will queue a {@link Runnable} onto the event queue thread for later invocation. */
public class TKTimerTask extends TimerTask {
	private static Timer	sTimer	= null;
	private Runnable		mRunnable;
	private boolean			mOnEventQueue;

	/**
	 * Creates a new timer task. The {@link Runnable} passed in will be invoked on the event queue
	 * thread.
	 * 
	 * @param runnable The {@link Runnable} to invoke.
	 */
	public TKTimerTask(Runnable runnable) {
		this(runnable, true);
	}
	 ...
	 */
	public TKTimerTask(Runnable runnable, boolean onEventQueue) {
		mRunnable = runnable;
	...
	public static void schedule(Runnable runnable, long delay, boolean onEventQueue) {
		getCommonTimer().schedule(new TKTimerTask(runnable, onEventQueue), delay);
	}
}
