 ...
 * extensions.
 */
public class TKWindowsRegistry implements Runnable {
	...
	public static final void register(String prefix, HashMap<String, String> map, File appFile, File iconDir) {
		if (TKPlatform.isWindows()) {
			Thread thread = new Thread(new TKWindowsRegistry(prefix, map, appFile, iconDir), TKWindowsRegistry.class.getSimpleName());
	...
	private TKWindowsRegistry(String prefix, HashMap<String, String> map, File appFile, File iconDir) {
		mPrefix = prefix;
		mMap = map;
		mAppFile = appFile;
		mIconDir = iconDir;
	...
	public void run() {
		try {
			...
			process = new TKProcess(new String[] { "regedit", "/S", regFile.getCanonicalPath() }); //$NON-NLS-1$ //$NON-NLS-2$
