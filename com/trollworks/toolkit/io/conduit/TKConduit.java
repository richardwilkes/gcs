...
public class TKConduit implements Runnable {
	 ...
	 */
	public TKConduit(TKConduitReceiver receiver, boolean onEventThread) {
	 ...
	 * @param onEventThread Pass in <code>true</code> to receive the messages on the event thread.
	 */
	public TKConduit(int port, TKConduitReceiver receiver, boolean onEventThread) {
	 ...
	 * @param onEventThread Pass in <code>true</code> to receive the messages on the event thread.
	 */
	public TKConduit(InetAddress address, int port, TKConduitReceiver receiver, boolean onEventThread) {
	 ...
	 * @param onEventThread Pass in <code>true</code> to receive the messages on the event thread.
	 */
	public TKConduit(InetSocketAddress socketAddress, TKConduitReceiver receiver, boolean onEventThread) {
		...
		mReceptionThread = new Thread(this, TKConduit.class.getSimpleName() + '@' + mSocketAddress);
	...
	private static final InetAddress getLoopBackAddress() {
		try {
			...
			assert false : TKDebug.throwableToString(uhe);
			return null;
		}
	}
	 ...
	 */
	public void send(TKConduitMessage msg) {
		while (true) {
			try {
				msg.send(mOutput);
				return;
	...
	public void run() {
		while (true) {
			try {
				TKConduitMessage msg = new TKConduitMessage(mInput);
	...
	private class Server extends Thread {
		...
		Server(InetSocketAddress socketAddress) throws IOException {
			super(TKConduit.class.getSimpleName() + '$' + Server.class.getSimpleName() + '@' + socketAddress);
		 ...
		 */
		void send(TKConduitMessage msg) {
			Client[] clients;
		...
		private class Client extends Thread {
			...
			Client(Socket socket) throws IOException {
				super(TKConduit.class.getSimpleName() + '$' + Client.class.getSimpleName() + '#' + getNextClientCounter() + '@' + getServerSocket().getLocalSocketAddress());
				setPriority(NORM_PRIORITY);
				setDaemon(true);
			...
			@Override public void run() {
				try {
					while (true) {
						Server.this.send(new TKConduitMessage(mClientInput));
					}
				} catch (Exception exception) {
