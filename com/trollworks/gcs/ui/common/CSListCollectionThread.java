...
public class CSListCollectionThread extends Thread {
	private static final CSListCollectionThread	INSTANCE;
	private File								mListDir;
	private ArrayList<Object>					mLists;
	private boolean								mRunning;

	static {
		INSTANCE = new CSListCollectionThread();
		INSTANCE.start();
	}

	/** @return The one and only instance of this thread. */
	public static final CSListCollectionThread get() {
		return INSTANCE;
	}

	private CSListCollectionThread() {
	...
	@Override public void run() {
		if (mRunning) {
			for (CSWindow window : TKWindow.getWindows(CSWindow.class)) {
				window.rebuildListMenu();
			}
		} else {
			mRunning = true;
			try {
			...
			if (files != null) {
				...
				for (File element : files) {
					if (element.isDirectory()) {
						...
						String ext = TKPath.getExtension(element.getName());

						if (CSListOpener.ADVANTAGE_EXTENSION.equalsIgnoreCase(ext) || CSListOpener.EQUIPMENT_EXTENSION.equalsIgnoreCase(ext) || CSListOpener.SKILL_EXTENSION.equalsIgnoreCase(ext) || CSListOpener.SPELL_EXTENSION.equalsIgnoreCase(ext) || CSTemplateOpener.EXTENSION.equalsIgnoreCase(ext)) {
							list.add(element);
						}
					}
				}
