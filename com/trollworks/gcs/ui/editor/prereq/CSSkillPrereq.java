...
public class CSSkillPrereq extends CSBasePrereq {
	private static final String	NAME_PREFIX				= "Name";			//$NON-NLS-1$
	private static final String	SPECIALIZATION_PREFIX	= "Specialization"; //$NON-NLS-1$
	 ...
	 */
	public CSSkillPrereq(CMRow row, CMSkillPrereq prereq, int depth) {
		super(row, prereq, depth);
	}

	@Override protected void rebuildSelf() {
		CMSkillPrereq prereq = (CMSkillPrereq) mPrereq;
		...
		wrapper = new TKPanel(new TKColumnLayout(2));
		addStringComparePopups(wrapper, prereq.getSpecializationCriteria(), Msgs.WHOSE_SPECIALIZATION, SPECIALIZATION_PREFIX);
		mCenter.add(wrapper);
	...
	@Override public boolean obeyCommand(String command, TKMenuItem item) {
		...
		if (command.startsWith(NAME_PREFIX)) {
			...
			handleStringCompareChange(prereq.getSpecializationCriteria(), command.substring(SPECIALIZATION_PREFIX.length()), null);
	...
	@Override public void actionPerformed(ActionEvent event) {
		...
		if (command.startsWith(NAME_PREFIX)) {
			...
			handleStringCompareChange(prereq.getSpecializationCriteria(), command.substring(SPECIALIZATION_PREFIX.length()), event);
		} else if (command.startsWith(LEVEL_PREFIX)) {
