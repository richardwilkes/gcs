...
public class CSSpellClassCell extends CSMultiCell {
	@Override protected String getPrimaryText(CMRow row) {
		return row.canHaveChildren() ? "" : ((CMSpell) row).getSpellClass(); //$NON-NLS-1$
	}

	@Override protected String getSecondaryText(CMRow row) {
		return row.canHaveChildren() ? "" : ((CMSpell) row).getCollege(); //$NON-NLS-1$
	}
}
