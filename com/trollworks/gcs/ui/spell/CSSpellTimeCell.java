...
public class CSSpellTimeCell extends CSMultiCell {
	@Override protected String getPrimaryText(CMRow row) {
		return row.canHaveChildren() ? "" : ((CMSpell) row).getCastingTime(); //$NON-NLS-1$
	}

	@Override protected String getSecondaryText(CMRow row) {
		return row.canHaveChildren() ? "" : ((CMSpell) row).getDuration(); //$NON-NLS-1$
	}
}
