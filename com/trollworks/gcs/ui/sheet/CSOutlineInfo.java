...
public class CSOutlineInfo {
	 ...
	 */
	public CSOutlineInfo(TKOutline outline) {
		Insets insets = (new TKBoxedDropShadowBorder()).getBorderInsets(null);
		TKOutlineModel outlineModel = outline.getModel();
		int count = outlineModel.getRowCount();
		List<TKColumn> columns = outlineModel.getColumns();
		...
		for (int i = 0; i < count; i++) {
			TKRow row = outlineModel.getRowAtIndex(i);
		...
		mOverheadHeight = insets.top + insets.bottom + outline.getHeaderPanel().getPreferredSize().height + (hasRowDividers ? 2 : 0);
