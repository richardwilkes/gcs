...
public class CMOldWeapon {
	...
	/**
	 * Creates a new {@link CMOldWeapon}.
	 * 
	 * @param reader The XML reader to load from.
	 * @throws IOException
	 */
	public CMOldWeapon(TKXMLReader reader) throws IOException {
	...
	private String[] extract(TKXMLReader reader) throws IOException {
		return TKTextUtility.extractLines(reader.readText().trim(), 0).toArray(new String[0]);
	}
	 ...
	 */
	public ArrayList<CMWeaponStats> getWeapons(CMRow owner) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (reach.length() > 0) {
				CMMeleeWeaponStats melee = new CMMeleeWeaponStats(owner);
			...
			if (range.length() > 0) {
				CMRangedWeaponStats ranged = new CMRangedWeaponStats(owner);
