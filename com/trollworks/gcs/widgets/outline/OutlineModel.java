...
public class OutlineModel implements Selection.Owner, StateEditable {
	...
	/** Creates a new model. */
	public OutlineModel() {
		mListeners = new ArrayList<OutlineModelListener>();
			...
			try {
				...
				if (NumberUtils.getInteger(tokenizer.nextToken().substring(1), 0) == CONFIG_VERSION) {
					...
					for (int i = 0; i < count; i++) {
						...
						column.setSortCriteria(NumberUtils.getInteger(tokenizer.nextToken(), -1), NumberUtils.getBoolean(tokenizer.nextToken()));
						if (column.getSortSequence() != -1) {
							result = 1;
						}
					}
	...
	public List<Row> getSelectionAsList(boolean minimal) {
		ArrayList<Row> list = new ArrayList<Row>(mSelection.getCount());
	...
	public void select(Collection<? extends Row> rows, boolean add) {
		HashSet<Row> set = new HashSet<Row>(rows);
		int[] indexes = new int[set.size()];
		int i = 0;
	...
	public void storeState(Hashtable<Object, Object> state) {
		...
		state.put(UNDO_KEY_ROWS, new ArrayList<Row>(rows));
	...
	@SuppressWarnings("unchecked") public void restoreState(Hashtable<?, ?> state) {
