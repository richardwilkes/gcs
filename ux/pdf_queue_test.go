...
// longer than should ever be needed, since it exists only so that a broken queue fails the test rather than hanging.
const pdfQueueHandshakeTimeout = 10 * time.Second

// fakePDFRenderer stands in for a *PDFRenderer so that the queue's ordering can be driven one step at a time. Each
// call to renderNext announces itself on started and then waits to be released, which keeps the single worker occupied
// -- and therefore the queue's state stable -- for as long as the test needs.
...
func (f *fakePDFRenderer) renderNext() {
	f.started <- f.name
	if <-f.release {
		// As PDFRenderer.recordPage does, the continuation is submitted before the render call returns.
...
// fatal, since every step that follows assumes the expected renderer is the one sitting in renderNext awaiting release.
...
func TestPDFQueueUserSignalBeatsContinuations(t *testing.T) {
	...
	// While it is rendering, a second document is opened. That submission is made before the busy renderer is
	// released, so the continuation the busy renderer makes for itself arrives after it.
	...
	// The busy document's continuation is still queued, so it resumes as soon as the newcomer is done.
...
func TestPDFQueueStickyPriority(t *testing.T) {
	...
	// Get the background document rendering, so the worker is busy while the rest of the queue's state is set up.
...
func TestPDFQueueRemoval(t *testing.T) {
	...
	// When it is removed it has a pending entry of its own, ahead of another document's entry in the backlog.
