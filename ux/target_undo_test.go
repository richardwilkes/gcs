...
// newTestCheckBox returns a CheckBox backed by state, registered with targetMgr under key and added to root.
...
// The editors rebuild their widgets whenever the model changes, so the widget that recorded an edit is usually gone by
// the time the edit is undone; the edit must go to whichever widget holds the target key at that moment.
