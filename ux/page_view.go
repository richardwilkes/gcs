...
// have in common: the file-backed panel at their root, the undo manager and UI scale of the document they show, and,
// around the page itself, the toolbar, the scroll panel it sits in, the target manager that finds its fields by
// reference key, and the search over its lists.
type pageView struct {
	...
	scale         int
	// awaitingUpdate is set while the dockable is being marked as modified; MarkModified does nothing while it is set
	// (see ownerRecalculates for one consequence of that).
	awaitingUpdate bool
}

// pageDockable is a dockable built on a pageView, as the shared scaffolding sees it: the file-backed dockable itself
// plus the parts the scaffolding calls back into, which each dockable defines for itself.
...
// been built: the content goes into the scroll panel, the toolbar is added above it, and the Save and Save As commands
// are wired up.
...
// attachedListProvider is a model that owns the items on its lists and matches them against the library sources they
// came from: a template or a loot sheet. An entity is one too, but readies itself as part of recalculating.
...
// been brought back into line with its model. That disturbs both parts of it: rebuilding a table takes the focus away
// from a field in it, and re-laying out the page moves the scroll position.
...
// rebuild its lists, recalculate its entity -- has been done: every panel is synced, the title is brought up to date,
// the search results, which name rows that may no longer exist, are found again, and the user's place is put back.
func (pv *pageView) resync(d unison.Dockable, state viewState) {
	DeepSync(d)
	UpdateTitleForDockable(d)
	// The update is over once every panel has been synced. What remains only puts the presentation back, and a change
	// reported from within it -- a field committing its edit as the focus leaves it -- is a fresh edit that calls for
	// an update of its own rather than one to be dropped.
	pv.awaitingUpdate = false
	pv.searchTracker.Refresh()
	pv.restoreViewState(state)
}

// markModified is the body of MarkModified for a dockable with a page view: the user's place is captured, prepare, if
// given, does whatever the dockable has to do first, and the page is brought back into line with the model. A call
// that arrives while the update is already under way is dropped.
