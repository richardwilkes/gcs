...
// reschedule brings the pending tick into line with the current setting and returns true if a tick is now pending. A
// pending tick already scheduled for the setting now in force is left alone, so re-applying unchanged settings --
// which the settings dialog does every time it syncs or resets -- doesn't push the next check further out each time.
// Switching from Never to anything else is the one change that runs the work immediately: Never is the only setting
// under which no check has been made this session, and a user who turns the checks on expects one to happen rather
// than waiting an hour, a day, or until the next launch. The first call, which starts the schedule at launch, doesn't
// count as turning the checks on, since the launch check is made separately.
...
// tick is scheduled before the work runs, both so the cadence is measured start-to-start and so a panic inside the
// work -- which unison recovers on the UI thread -- doesn't silently end the chain. A tick that finds the setting no
// longer calls for repeating checks schedules nothing further and doesn't run the work either; this is a backstop,
// since every path that changes the setting goes through ApplyUpdateCheckSettings.
...
// appUpdateCheckFrequency returns the app update check setting as it applies to this build. A development build has no
// release behind it to compare against, so there is no point in waking up every hour or day to say so again: for it,
// the repeating options are treated as checking only at launch.
