...
// sticky priority -- belongs to that goroutine and is never touched elsewhere, so these messages are the only way to
...
type pdfQueueParams struct {
	...
	// no renderer. Routing this through the channel the submissions arrive on is what makes the ordering airtight: a
	// renderer submits its continuation before its render call returns, and therefore before this message is sent, so
...
// newPDFQueue creates a render queue along with the goroutines that drive it. Only one page is ever rendered at a
// time. pdfview is pure Go and serializes rendering within a single document internally, so this isn't needed for
// correctness -- the single worker bounds the memory churn rendering many large pages at once would cause.
...
// someone is looking at right now. No page number is passed along: the renderer picks the highest priority page that
// still needs rendering when the work actually starts, so the newest visibility information always wins.
...
//
// The send can't deadlock, even from the UI thread while the worker is in the middle of a render: process() only waits
// on q.in and on the handoff to the worker, never on anything a renderer does, so it always comes back around to drain
// q.in. The worst case is that this waits for room in the buffer.
...
func (q *pdfQueueData) process() {
	...
	for {
		if idle && len(q.backlog) != 0 {
			i := q.dispatchIndex()
			// The worker reports itself idle before returning to its receive, so at worst this waits for it to get
			// back there.
			q.work <- q.backlog[i]
...
// signal goes to the front, since someone is looking at that document right now, while a continuation goes to the back
// and leaves an entry that is already present where it is. That is what keeps a document which is still catching up
// from starving one that was just opened.
