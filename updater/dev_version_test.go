...
// A plain `go build` from a git checkout stamps a VCS pseudo-version, which sorts above the latest release, so failing
// to recognize one would send such a build off to check for updates and then report that there were none.
...
// The versions the release workflow stamps must not be mistaken for development builds, including a pre-release tag,
// which looks like the start of a pseudo-version but isn't one.
