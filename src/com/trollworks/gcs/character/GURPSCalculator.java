...
public class GURPSCalculator {
    ...
    private static boolean showExistsDialogIfNecessary(WebServiceClient client, GURPSCharacter character) throws MalformedURLException, IOException, NotImplementedException {
        if (client.sendRequest(HttpMethodType.GET, String.format("api/GetCharacterExists/%s/%s", character.getId(), OutputPreferences.getGurpsCalculatorKey())).equals("true")) {
    ...
    private static void showResult(boolean success) {
        ...
        buffer.append(font.isBold() ? "bold" : "normal");
        buffer.append(";font-size:");
        buffer.append(font.getSize());
        buffer.append("pt;background-color: rgb(");
        ...
        JEditorPane messagePane = new JEditorPane("text/html", buffer.toString());
