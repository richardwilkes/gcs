...
public class GCSMain {
	...
	private static int convert(CmdLine cmdLine) {
		boolean html = cmdLine.isOptionUsed(GCSMain.HTML_OPTION);
		boolean pdf = cmdLine.isOptionUsed(GCSMain.PDF_OPTION);
		boolean png = cmdLine.isOptionUsed(GCSMain.PNG_OPTION);
		int count = 0;
		...
		if (html || pdf || png) {
			...
			Timing timing = new Timing();
			String htmlTemplateOption = cmdLine.getOptionArgument(GCSMain.HTML_TEMPLATE_OPTION);
	...
	private static double[] getPaperSize(CmdLine cmdLine) {
		if (cmdLine.isOptionUsed(GCSMain.SIZE_OPTION)) {
			String argument = cmdLine.getOptionArgument(GCSMain.SIZE_OPTION);
			int index;
	...
	private static double[] getMargins(CmdLine cmdLine) {
		if (cmdLine.isOptionUsed(GCSMain.MARGIN_OPTION)) {
			StringTokenizer tokenizer = new StringTokenizer(cmdLine.getOptionArgument(GCSMain.MARGIN_OPTION), ":"); //$NON-NLS-1$
