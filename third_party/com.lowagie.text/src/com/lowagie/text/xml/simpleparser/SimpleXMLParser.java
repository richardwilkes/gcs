...
/**
 * A simple XML and HTML parser. This parser is, like the SAX parser, an event based parser, but
 ...
 * <li>It recognizes the standard entities: &amp;amp;, &amp;lt;, &amp;gt;, &amp;quot;, and
 * &amp;apos;, as well as numeric entities
 * <li>It maps lines ending in <code>\r\n</code> and <code>\r</code> to <code>\n</code> on input, in
 * accordance with the XML Specification, Section 2.11
 * </ul>
 * <p>
 */
...
public final class SimpleXMLParser {
	...
	boolean						eol					= false;
	/**
	 * A boolean indicating if the next character should be taken into account if it's a space
	 * character. When nospace is false, the previous character wasn't whitespace.
	 * 
	 * @since 2.1.5
	 */
	...
	/**
	 * Flushes the text that is currently in the buffer. The text can be ignored, added to the
	 * document as content or as comment,... depending on the current state.
	 */
	private void flush() {
		switch (state) {
			case TEXT:
	 ...
	 * processes the tag.
	 * 
	 * @param start if true we are dealing with a tag that has just been opened; if false we are
	 *            closing a tag.
	 */
	 ...
	 * endian-ness of that encoding where appropriate. (method found in
	 * org.apache.xerces.impl.XMLEntityManager, originally published by the Apache Software
	 * Foundation under the Apache Software License; now being used in iText under the MPL)
	 * 
