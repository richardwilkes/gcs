...
/**
 * This class is a Handler that controls the iText XML to PDF conversion. Subclass it, if you want
 * to change the way iText translates XML to PDF.
 */
...
public class SAXiTextHandler extends DefaultHandler {
	...
	/**
	 * Sets the parameter that allows you to enable/disable the control over the Document.open() and
	 * Document.close() method.
	 * <P>
	 * If you set this parameter to true (= default), the parser will open the Document object when
	 * the start-root-tag is encountered and close it when the end-root-tag is met. If you set it to
	 * false, you have to open and close the Document object yourself.
	 * 
	...
	public void handleStartingTags(String name, Properties attributes) {
		...
		if (currentChunk != null) {
			...
			try {
				...
				if (bf == null) {
					...
					current = new Paragraph("", new Font(bf));
				}
			}
		...
		if (ElementTags.CHUNK.equals(name)) {
			...
			if (bf != null) {
				currentChunk.setFont(new Font(bf));
			}
			return;
		}
		...
		if (isDocumentRoot(name)) {
			...
			for (Object element : attributes.keySet()) {
				key = (String) element;
	...
	public void characters(char[] ch, int start, int length) {
		...
		if (currentChunk == null) {
			if (bf == null) {
				...
				currentChunk = new Chunk(buf.toString(), new Font(bf));
			}
		} else {
