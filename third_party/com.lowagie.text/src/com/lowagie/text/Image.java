...
public abstract class Image extends Rectangle {
		...
		try {
			...
			if (c1 == 0x97 && c2 == 'J' && c3 == 'B' && c4 == '2') {
				...
				if (c5 == '\r' && c6 == '\n' && c7 == 0x1a && c8 == '\n') {
					...
					// embedded jbig2s don't have a header, have to create them by explicit use of
					// Jbig2Image?
	...
	/**
	 * Creates an Image with CCITT G3 or G4 compression. It assumes that the data bytes are already
	 * compressed.
	 *
	 * @param width the exact width of the image
	 * @param height the exact height of the image
	 * @param reverseBits reverses the bits in <code>data</code>. Bit 0 is swapped with bit 7 and so
	 *            on
	 * @param typeCCITT the type of compression in <code>data</code>. It can be CCITTG4, CCITTG31D,
	 *            CCITTG32D
	 * @param parameters parameters associated with this stream. Possible values are CCITT_BLACKIS1,
	 *            CCITT_ENCODEDBYTEALIGN, CCITT_ENDOFLINE and CCITT_ENDOFBLOCK or a combination of
	 *            them
	 * @param data the image data
	 * @return an Image object
	 * @throws BadElementException on error
	 ...
	 * @param color if different from <CODE>null</CODE> the transparency pixels are replaced by this
	 *            color
	...
	/**
	 * Holds value of property directReference. An image is embedded into a PDF as an Image XObject.
	...
	/**
	 * Sets the image interpolation. Image interpolation attempts to produce a smooth transition
