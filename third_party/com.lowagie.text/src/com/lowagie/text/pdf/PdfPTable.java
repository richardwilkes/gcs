...
/**
 * This is a table that can be put at an absolute position but can also be added to the document as
 * the class <CODE>Table</CODE>. In the last case when crossing pages the table always break at full
 * rows; if a row is bigger than the page it is dropped silently to avoid infinite loops.
 * <P>
...
public class PdfPTable implements LargeElement {
	 ...
	 * 
	 * @param firsttime if true, the heights of the rows will be recalculated. This takes time;
	 *            normally the heights of the rows are already calcultated, so in most cases, it's
	 *            save to use false as parameter.
	 * @return the total height of the table. Note that it will be 0 if you didn't specify the width
	 *         of the table with setTotalWidth().
	...
	/**
	 * Gets the default <CODE>PdfPCell</CODE> that will be used as reference for all the
	 * <CODE>addCell</CODE> methods except <CODE>addCell(PdfPCell)</CODE>.
	 * 
	...
	/**
	 * When updating the row index, cells with rowspan should be taken into account. This is what
	...
	/**
	 * Writes the selected rows to the document. <CODE>canvases</CODE> is obtained from
	 ...
	 * this is only important if there are columns with colspan at boundaries. <CODE>canvases</CODE>
	 * is obtained from <CODE>beginWritingRows()</CODE>. The table event is only fired for complete
	 * rows.
	 * 
	...
	/**
	 * Writes the selected rows and columns to the document. This method clips the columns; this is
	 * only important if there are columns with colspan at boundaries. The table event is only fired
	 * for complete rows.
	 * 
	 * @param colStart the first column to be written, zero index
	 * @param colEnd the last column to be written + 1. If it is -1 all the columns to the end are
	...
	/**
	 * Gets and initializes the 4 layers where the table is written to. The text or graphics are
	 * added to one of the 4 <CODE>PdfContentByte</CODE> returned with the following order:
	 * <p>
	 * <ul>
	 * <li><CODE>PdfPtable.BASECANVAS</CODE> - the original <CODE>PdfContentByte</CODE>. Anything
	 * placed here will be under the table.
	...
	/**
	 * Gets the maximum height of a cell in a particular row (will only be different from
	 * getRowHeight is one of the cells in the row has a rowspan > 1).
	 * 
	...
	/**
	 * Sets the number of the top rows that constitute the header. This header has only meaning if
	 * the table is added to <CODE>Document</CODE> and the table crosses pages.
	 * 
	...
	/**
	 * Sets the horizontal alignment of the table relative to the page. It only has meaning if the
	 * width percentage is less than 100%.
	 *
	...
	public void setTableEvent(PdfPTableEvent event) {
		if (event == null) {
			...
			tableEvent = event;
		} else if (tableEvent instanceof PdfPTableEventForwarder) {
	...
	/**
	 * Tells you if the first header needs to be skipped (for instance if the header says "continued
	...
	/**
	 * Tells you if the last footer needs to be skipped (for instance if the footer says "continued
	 * on the next page")
	 * 
	...
	/**
	 * Skips the printing of the first header. Used when printing tables in succession belonging to
	 * the same printed table aspect.
	 * 
	...
	/**
	 * Skips the printing of the last footer. Used when printing tables in succession belonging to
	 * the same printed table aspect.
	 * 
	...
	/**
	 * When set the rows that won't fit in the page will be split. Note that it takes at least twice
	 * the memory to handle a split table row than a normal table. <CODE>true</CODE> by default.
	 * 
	...
	/**
	 * When set the last row will be extended to fill all the remaining space to the bottom
	 * boundary.
	 * 
	...
	/**
	 * If true the table will be kept on one page if it fits, by forcing a new page if it doesn't
	 * fit on the current page. The default is to split the table over multiple pages.
	 *
	...
	/**
	 * Sets the number of rows to be used for the footer. The number of footer rows are subtracted
	 * from the header rows. For example, for a table with two header rows and one footer row the
	 * code would be:
	 * 
	 * <pre>
	...
	/**
	 * Completes the current row with the default cell. An incomplete row will be dropped but
	 * calling this method will make sure that it will be present in the table.
	 */
