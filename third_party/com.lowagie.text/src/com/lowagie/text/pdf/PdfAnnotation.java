...
public class PdfAnnotation extends PdfDictionary {
	...
	public static PdfAnnotation createInk(PdfWriter writer, Rectangle rect, String contents, float inkList[][]) {
		...
		for (float[] deep : inkList) {
			...
			for (float element : deep) {
				inner.add(new PdfNumber(element));
			}
			outer.add(inner);
		}
	...
	/**
	 * Sets the annotation's highlighting mode. The values can be <CODE>HIGHLIGHT_NONE</CODE>,
	 * <CODE>HIGHLIGHT_INVERT</CODE>, <CODE>HIGHLIGHT_OUTLINE</CODE> and
	 * <CODE>HIGHLIGHT_PUSH</CODE>;
	 * 
	 ...
	 * With <code>PdfStamper</code> the page can be any. The first page is 1.
	 * 
	...
	/**
	 * Sets the name of the annotation. With this name the annotation can be identified among all
	 * the annotations on a page (it has to be unique).
	 */
	 ...
	 * example code reads a group of files and places them all on the output PDF, four pages in a
	 * single page, keeping the links active.
	 * 
	 * <pre>
