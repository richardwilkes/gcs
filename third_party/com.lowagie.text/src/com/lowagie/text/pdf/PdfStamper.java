...
/**
 * Applies extra content to the pages of a PDF document. This extra content can be all the objects
 * allowed in PdfContentByte including pages from other Pdfs. The original PDF will keep all the
 * interactive elements including bookmarks, links and form fields.
 * <p>
 * It is also possible to change the field values and to flatten them. New fields can be added but
 * not flattened.
 *
...
public class PdfStamper implements PdfViewerPreferences {
	...
	/**
	 * Starts the process of adding extra content to an existing PDF document, possibly as a new
	 * revision.
	 *
	 * @param reader the original document. It cannot be reused
	 * @param os the output stream
	...
	/**
	 * An optional <CODE>String</CODE> map to add or change values in the info dictionary. Entries
	 * with <CODE>null</CODE> values delete the key in the original info dictionary
	 *
	 ...
	 * or getUndercontent() are called for the same page.
	 *
	...
	/**
	 * Inserts a blank page. All the pages above and including <CODE>pageNumber</CODE> will be
	 * shifted up. If <CODE>pageNumber</CODE> is bigger than the total number of pages the new page
	...
	/**
	 * Flags the content to be automatically adjusted to compensate the original page rotation. The
	 * default is <CODE>true</CODE>.
	 *
	 * @param rotateContents <CODE>true</CODE> to set auto-rotation, <CODE>false</CODE> otherwise
	 */
	...
	/**
	 * Gets a page from other PDF document. Note that calling this method more than once with the
	 * same parameters will retrieve the same object.
	 *
	...
	/**
	 * Gets the <CODE>AcroFields</CODE> object that allows to get and set field values and to merge
	 * FDF forms.
	 *
	 ...
	 *
	 * @param flat <CODE>true</CODE> to flatten the fields, <CODE>false</CODE> to keep the fields
	 */
	 ...
	 *
	 * @param flat <CODE>true</CODE> to flatten the FreeText annotations, <CODE>false</CODE> (the
	 *            default) to keep the FreeText annotations as active content.
	 */
	...
	/**
	 * Adds <CODE>name</CODE> to the list of fields that will be flattened on close, all the other
	 * fields will remain. If this method is never called or is called with invalid field names, all
	 ...
	 * runs. The existing JavaScript will be replaced.
	 *
	...
	/**
	 * This is the most simple way to change a PDF into a portable collection. Choose one of the
	 * following names:
	 * <ul>
	 ...
	 * <li>PdfName.H (hidden)
	 * </ul>
	 * Pass this name as a parameter and your PDF will be a portable collection with all the
	 * embedded and attached files as entries.
	 *
	...
	/**
	 * Sets the document's compression to the new 1.5 mode with object streams and xref streams. It
	 ...
	 * multiple signatures. The returned PdfStamper can be used normally as the signature is only
	 ...
	 * @param pdfVersion the new pdf version or '\0' to keep the same version as the original
	 *            document
	 * @param tempFile location of the temporary file. If it's a directory a temporary file will be
	 *            created there. If it's a file it will be used directly. The file will be deleted
	 *            on exit unless <CODE>os</CODE> is null. In that case the document can be retrieved
	 *            directly from the temporary file. If it's <CODE>null</CODE> no temporary file will
	 *            be created and memory will be used
	...
	/**
	 * Applies a digital signature to a document. The returned PdfStamper can be used normally as
	 * the signature is only applied when closing.
	 * <p>
