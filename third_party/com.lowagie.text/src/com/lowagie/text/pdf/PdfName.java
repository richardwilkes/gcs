 ...
 * <P>
 * A name, like a string, is a sequence of characters. It must begin with a slash followed by a
 * sequence of ASCII characters in the range 32 through 136 except %, (, ), [, ], &lt;, &gt;, {, },
 * / and #. Any character except 0x00 may be included in a name by writing its two character hex
 * code, preceded by #. The maximum number of characters in a name is 127.<BR>
...
public class PdfName extends PdfObject implements Comparable {
	...
	/**
	 * Use reflection to cache all the static public final names so future <code>PdfName</code>
	 * additions don't have to be "added twice". A bit less efficient (around 50ms spent here on a
	 * 2.2ghz machine), but Much Less error prone.
	 * 
	 * @since 2.1.6
	 */
	 ...
	 * 
	 * @param name the new name
	 * @param lengthCheck if <CODE>true</CODE> check the length validity, if <CODE>false</CODE> the
	 *            name can have any length
	 */
	public PdfName(String name, boolean lengthCheck) {
		super(PdfObject.NAME);
		// The minimum number of characters in a name is 0, the maximum is 127 (the '/' not
		// included)
