 ...
 * <P>
 * When using variable borders ({@link com.lowagie.text.Rectangle#isUseVariableBorders
 * isUseVariableBorders()} == true), the borders are drawn completely inside the cell Rectangle so
 * that adjacent cell borders will not overlap. Otherwise, the borders are drawn on top of the edges
 * of the cell Rectangle and will overlap the borders of adjacent cells.
 *
...
public class PdfCell extends Rectangle {
	...
	/**
	 * This is the total height of the content of the cell. Note that the actual cell height may be
	 * larger due to another cell on the row *
	 */
	private float		contentHeight	= 0.0f;

	/**
	 * Indicates that the largest ascender height should be used to determine the height of the
	 * first line. Setting this to true can help with vertical alignment problems.
	 */
	private boolean		useAscender;

	/**
	 * Indicates that the largest descender height should be added to the height of the last line
	 * (so characters like y don't dip into the border).
	 */
	...
	public PdfCell(Cell cell, int rownumber, float left, float right, float top, float cellspacing, float cellpadding) {
		...
		useAscender = cell.isUseAscender();
		useDescender = cell.isUseDescender();
		useBorderPadding = cell.isUseBorderPadding();
		...
		for (Iterator i = cell.getElements(); i.hasNext();) {
			...
			switch (element.type()) {
				...
				default:
					...
					if (chunks.isEmpty()) {
						addLine(line); // add empty line - all cells need some lines even if they
										 // are empty
						line = new PdfLine(currentLeft, currentRight, alignment, currentLineLeading);
					} else {
					...
					// if the element is a paragraph, section or chapter, we reset the alignment and
					// add the line
					switch (element.type()) {
	 ...
	 * the top. For text, this is the height of the ascender. For an image, it is the actual height
	 * of the image.
	 * 
	...
	/**
	 * Gets the amount of the border for the specified side that is inside the Rectangle. For
	 * non-variable width borders this is only 1/2 the border width on that side. This always
	 * returns 0 if {@link #useBorderPadding} is false;
	 * 
	 * @param side the side to check. One of the side constants in
	 *            {@link com.lowagie.text.Rectangle}
	...
	public ArrayList getLines(float top, float bottom) {
		...
		// if the bottom of the cell is higher than the bottom of the page, the cell is written, so
		// we can remove all lines
		float difference = 0f;
		if (!header) {
			if (aboveBottom) {
				lines = new ArrayList();
	 ...
	 * <P>
	 * Headers may always be removed, even if they are drawn only partially: they will be repeated
	 * on each following page anyway!
	 *
