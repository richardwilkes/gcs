...
/**
 * An optional content group is a dictionary representing a collection of graphics that can be made
 * visible or invisible dynamically by users of viewer applications. In iText they are referenced as
 * layers.
 *
...
public class PdfLayer extends PdfDictionary implements PdfOCG {
	...
	/**
	 * Creates a title layer. A title layer is not really a layer but a collection of layers under
	 * the same title heading.
	 * 
	...
	/**
	 * Sets the <CODE>PdfIndirectReference</CODE> that represents this layer. This can only be done
	 * from PdfStamperImp.
	 * 
	...
	/**
	 * Used by the creating application to store application-specific data associated with this
	 ...
	 *            values include but are not limited to <B>Artwork</B>, for graphic-design or
	 *            publishing applications, and <B>Technical</B>, for technical designs such as
	 *            building plans or schematics
	 */
	 ...
	 * 
	 * @param lang a language string which specifies a language and possibly a locale (for example,
	 *            <B>es-MX</B> represents Mexican Spanish)
	 * @param preferred used by viewer applications when there is a partial match but no exact match
	 *            between the system language and the language strings in all usage dictionaries
	 */
	...
	/**
	 * Specifies the recommended state for content in this group when the document (or part of it)
	 * is saved by a viewer application to a format that does not support optional content (for
	 * example, an earlier version of PDF or a raster image format).
	 * 
	...
	/**
	 * Specifies a range of magnifications at which the content in this optional content group is
	 * best viewed.
	 * 
	 * @param min the minimum recommended magnification factors at which the group should be ON. A
	 *            negative value will set the default to 0
	 * @param max the maximum recommended magnification factor at which the group should be ON. A
	 *            negative value will set the largest possible magnification supported by the viewer
	 *            application
	 */
	 ...
	 * 
	 * @param subtype a name specifying the kind of content controlled by the group; for example,
	 *            <B>Trapping</B>, <B>PrintersMarks</B> and <B>Watermark</B>
	 * @param printstate indicates that the group should be set to that state when the document is
	 *            printed from a viewer application
	 */
	...
	/**
	 * Indicates that the group should be set to that state when the document is opened in a viewer
	 * application.
	 * 
	...
	/**
	 * Sets the visibility of the layer in Acrobat's layer panel. If <CODE>false</CODE> the layer
	 * cannot be directly manipulated by the user. Note that any children layers will also be absent
	 * from the panel.
	 * 
