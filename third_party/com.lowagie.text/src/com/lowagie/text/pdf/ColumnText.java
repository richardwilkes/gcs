...
/**
 * Formats text in a columnwise form. The text is bound on the left and on the right by a sequence
 * of lines. This allows the column to have any shape, not only rectangular.
 * <P>
 * Several parameters can be set like the first paragraph line indent and extra space between
 * paragraphs.
 * <P>
 * A call to the method <CODE>go</CODE> will return one of the following situations: the column
 * ended or the text ended.
 * <P>
 * I the column ended, a new column definition can be loaded with the method <CODE>setColumns</CODE>
 * and the method <CODE>go</CODE> can be called again.
 * <P>
 * If the text ended, more text can be loaded with <CODE>addText</CODE> and the method
 * <CODE>go</CODE> can be called again.<BR>
 ...
 * <CODE>PdfWriter.RUN_DIRECTION_RTL</CODE> the meaning of the horizontal alignments and margins is
 * mirrored.
 * 
...
public class ColumnText {
	...
	/**
	 * Adds an element. Elements supported are <CODE>Paragraph</CODE>, <CODE>List</CODE>,
	 * <CODE>PdfPTable</CODE>, <CODE>Image</CODE> and <CODE>Graphic</CODE>.
	 * <p>
	...
	/**
	 * Finds the intersection between the <CODE>yLine</CODE> and the two column bounds. It will set
	 * the <CODE>lineStatus</CODE> appropriately.
	 * 
	...
	/**
	 * Finds the intersection between the <CODE>yLine</CODE>, the <CODE>yLine-leading</CODE>and the
	 * two column bounds. It will set the <CODE>lineStatus</CODE> appropriately.
	 * 
	...
	/**
	 * Sets the columns bounds. Each column bound is described by a <CODE>float[]</CODE> with the
	 * line points [x1,y1,x2,y2,...]. The array must have at least 4 elements.
	 * 
	...
	/**
	 * Gets the width that the line will occupy after writing. Only the width of the first line is
	 * returned.
	 * 
		...
		while (true) {
			...
			if (element.type() == Element.PARAGRAPH) {
				...
				if (!simulate) {
					...
					// otherwise no footers will be added (because iText thinks the table continues
					// on the same page)
	...
	/**
	 * Sets the real width used by the largest line. Only used to set it to zero to start another
	 * measurement.
	 *
	 ...
	 * behave differently if the object is the first to be written after go() or not. The first line
	 * adjustment is <CODE>true</CODE> by default but can be changed if several objects are to be
	 * placed one after the other in the same column calling go() several times.
	 * 
	 * @param adjustFirstLine <CODE>true</CODE> to adjust the first line, <CODE>false</CODE>
	 *            otherwise
	 */
