...
public class CFFFont {
	...
	protected static abstract class OffsetItem extends Item {
		...
		/**
		 * set the value of an offset item that was initially unknown. It will be fixed up latex by
		 * a call to xref on some marker.
		 */
	...
	/**
	 * An index-offset item for the list. The size denotes the required size in the CFF. A positive
	 * value means that we need a specific size in bytes (for offset arrays) and a negative value
	 * means that this is a dict item that uses a variable-size representation.
	 */
	...
	static protected final class IndexMarkerItem extends Item {
		...
		@Override
		public void xref() {
			// System.err.println("index marker item, base="+indexBase.myOffset+"
			// my="+this.myOffset);
	...
	/**
	 * an unknown offset in a dictionary for the list. We will fix up the offset later; for now,
	 * assume it's large.
	 */
	...
	/**
	 * A dictionary number on the list. This implementation is inefficient: it doesn't use the
	...
	/**
	 * An offset-marker item for the list. It is used to mark an offset and to set the offset list
	 * item.
	 */
	...
	/**
	 * get a single CID font. The PDF architecture (1.4) supports 16-bit strings only with CID CFF
	 * fonts, not in Type-1 CFF fonts, so we convert the font to CID if it is in the Type-1 format.
	 * Two other tasks that we need to do are to select only a single font from the CFF package
	 * (this again is a PDF restriction) and to subset the CharStrings glyph description.
	 */
	...
	// throws java.io.FileNotFoundException
	{
		...
		if (fonts[j].isCID) {
			...
			if (fonts[j].privateSubrs >= 0) {
				// System.err.println("has subrs="+fonts[j].privateSubrs+"
				// ,len="+fonts[j].privateLength);
