...
public abstract class BaseFont {
     ...
     *
     * @param key the full name of the resource
     * @param module the Module to load the resource or null to try the ones available
     * @return the <CODE>InputStream</CODE> to get the resource or <CODE>null</CODE> if not found
     */
    public static InputStream getResourceStream(String key, Module module) {
        if (key.startsWith("/")) {
            key = key.substring(1);
        }
        InputStream is = null;
        if (module != null) {
            try {
                is = module.getResourceAsStream(key);
                if (is != null) {
                    return is;
                }
        ...
        if (is == null) {
            try {
                is = BaseFont.class.getModule().getResourceAsStream("/" + key);
            } catch (IOException ioe) {
                // Try next
            }
        }
