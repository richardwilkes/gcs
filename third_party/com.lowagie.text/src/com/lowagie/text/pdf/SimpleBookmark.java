...
/**
 * Bookmark processing in a simple way. It has some limitations, mainly the only action types
 * supported are GoTo, GoToR, URI and Launch.
 * <p>
 * The list structure is composed by a number of HashMap, keyed by strings, one HashMap for each
 * bookmark. The element values are all strings with the exception of the key "Kids" that has
 * another list for the child bookmarks.
 * <p>
 * All the bookmarks have a "Title" with the bookmark title and optionally a "Style" that can be
 * "bold", "italic" or a combination of both. They can also have a "Color" key with a value of three
 * floats separated by spaces. The key "Open" can have the values "true" or "false" and signals the
 * open status of the children. It's "true" by default.
 * <p>
...
public final class SimpleBookmark implements SimpleXMLDocHandler {
	...
	private static void mapGotoBookmark(HashMap map, PdfObject dest, IntHashtable pages) {
		if (dest.isString()) {
			...
			map.put("Page", makeBookmarkParam((PdfArray) dest, pages)); // changed by ujihara
																		 // 2004-06-13
		}
		map.put("Action", "GoTo");
	}
	...
	private static String makeBookmarkParam(PdfArray dest, IntHashtable pages) {
		...
		if (obj.isNumber()) {
			...
			s.append(pages.get(getNumber((PdfIndirectReference) obj))); // changed by ujihara
																		 // 2004-06-13
		}
	...
	/**
	 * Gets number of indirect. If type of directed indirect is PAGES, it refers PAGE object through
	 * KIDS. (Contributed by Kazuya Ujihara)
	 * 
	...
	/**
	 * Gets a <CODE>List</CODE> with the bookmarks. It returns <CODE>null</CODE> if the document
	 * doesn't have any bookmarks.
	 * 
	 * @param reader the document
	 * @return a <CODE>List</CODE> with the bookmarks or <CODE>null</CODE> if the document doesn't
	 *         have any
	 */
	...
	/**
	 * For the pages in range add the <CODE>pageShift</CODE> to the page number. The page ranges
	 * consists of a number of pairs with the start/end page range. The page numbers are inclusive.
	 * 
	 * @param list the bookmarks
	 * @param pageShift the number to add to the pages in range
	 * @param pageRange the page ranges, always in pairs. It can be <CODE>null</CODE> to include all
	 *            the pages
	 */
	...
	/**
	 * Exports the bookmarks to XML. Only of use if the generation is to be include in some other
	 * XML document.
	 * 
	 * @param list the bookmarks
