...
/**
 * Formats content into one or more columns bounded by a rectangle. The columns may be simple
 * rectangles or more complicated shapes. Add all of the columns before adding content. Column
 * continuation is supported. A MultiColumnText object may be added to a document using
...
public class MultiColumnText implements Element {
	...
	/**
	 * Top of the columns - y position on starting page. If <CODE>AUTOMATIC</CODE>, it means current
	 * y position when added to document
	 */
	private float				top;
	...
	/**
	 * Default constructor. Sets height to <CODE>AUTOMATIC</CODE>. Columns will repeat on each page
	 * as necessary to accommodate content length.
	 */
	...
	/**
	 * Construct a MultiColumnText container of the specified height. If height is
	 * <CODE>AUTOMATIC</CODE>, fill complete pages until done. If a specific height is used, it may
	...
	/**
	 * Construct a MultiColumnText container of the specified height starting at the specified Y
	 * position.
	 *
	 * @param height
	 * @param top
	...
	/**
	 * Indicates that all of the text did not fit in the specified height. Note that isOverflow will
	 * return false before the MultiColumnText object has been added to the document. It will always
	 * be false if the height is AUTOMATIC.
	 *
	...
	/**
	 * Add a new column. The parameters are limits for each column wall in the format of a sequence
	...
	/**
	 * Add the specified number of evenly spaced rectangular columns. Columns will be separated by
	 * the specified gutterWidth.
	 *
	...
	/**
	 * Write out the columns. After writing, use {@link #isOverflow()} to see if all text was
	 * written.
	 * 
	...
	public float write(PdfContentByte canvas, PdfDocument document, float documentY) throws DocumentException {
		...
		try {
			while (!done) {
				if (top == AUTOMATIC) {
					top = document.getVerticalPosition(true); // RS - 07/07/2005 - Get current doc
																 // writing position for top of
																 // columns on new page.
				} else if (nextY == AUTOMATIC) {
					nextY = document.getVerticalPosition(true); // RS - 07/07/2005 - - Get current
																 // doc writing position for top of
																 // columns on new page.
				}
	...
	private class ColumnDef {
		...
		/**
		 * Resolves the positions for the specified side of the column into real numbers once the
		 * top of the column is known.
		 *
