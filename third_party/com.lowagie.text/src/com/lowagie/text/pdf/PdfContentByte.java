...
/**
 * <CODE>PdfContentByte</CODE> is an object containing the user positioned text and graphic contents
...
public class PdfContentByte {
	 ...
	 * <P>
	 * The <VAR>line cap style</VAR> specifies the shape to be used at the end of open subpaths when
	 * they are stroked.<BR>
	 ...
	 * specified by an <I>array</I> and a <I>phase</I>. The array specifies the length of the
	 * alternating dashes and gaps. The phase specifies the distance into the dash pattern to start
	 * the dash.<BR>
	 *
	 ...
	 *
	 * @param phase the value of the phase
	 * @param unitsOn the number of units that must be 'on' (equals the number of units that must be
	 *            'off').
	 */
	 ...
	 * <P>
	 * The <VAR>line join style</VAR> specifies the shape to be used at the corners of paths that
	 * are stroked.<BR>
	 * Allowed values are LINE_JOIN_MITER (Miter joins), LINE_JOIN_ROUND (Round joins) and
	 * LINE_JOIN_BEVEL (Bevel joins).<BR>
	 *
	 * @param style a value
	 */
	 ...
	 * <P>
	 * The line width specifies the thickness of the line used to stroke a path and is measured in
	 * user space units.<BR>
	 *
	 * @param w a width
	 */
	 ...
	 * <P>
	 * Sets the color space to <B>DeviceGray</B> (or the <B>DefaultGray</B> color space), and sets
	 * the gray tint to use for filling paths.
	 * </P>
	 *
	 ...
	 * <P>
	 * Sets the color space to <B>DeviceGray</B> (or the <B>DefaultGray</B> color space), and sets
	 * the gray tint to use for stroking paths.
	 * </P>
	 *
	 * @param gray a value between 0 (black) and 1 (white)
	 */
	 ...
	 * <P>
	 * Sets the color space to <B>DeviceRGB</B> (or the <B>DefaultRGB</B> color space), and sets the
	 * color to use for filling paths.
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0 (minimum intensity) and 1
	 * (maximum intensity).
	 * </P>
	 *
	 * @param red the intensity of red. A value between 0 and 1
	 ...
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0 (miniumum intensity) and 1
	 * (maximum intensity).
	 *
	 * @param red the intensity of red. A value between 0 and 1
	 * @param green the intensity of green. A value between 0 and 1
	 ...
	 * Changes the current color for filling paths (device dependent colors!).
	 * <P>
	 * Sets the color space to <B>DeviceCMYK</B> (or the <B>DefaultCMYK</B> color space), and sets
	 * the color to use for filling paths.
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0 (no ink) and 1 (maximum
	 * ink).
	 * </P>
	 *
	 * @param cyan the intensity of cyan. A value between 0 and 1
	 ...
	 * <P>
	 * Sets the color space to <B>DeviceCMYK</B> (or the <B>DefaultCMYK</B> color space), and sets
	 * the color to use for stroking paths.
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0 (miniumum intensity) and 1
	 * (maximum intensity).
	...
	/**
	 * Appends a straight line segment from the current point <I>(x, y)</I>. The new current point
	 * is <I>(x, y)</I>.
	 *
	 * @param x new x-coordinate
	 * @param y new y-coordinate
	 ...
	 * {@link com.lowagie.text.Rectangle#isUseVariableBorders() Rectangle.isUseVariableBorders} =
	 * true.
	 * 
	...
	/**
	 * Fills the path using the non-zero winding number rule to determine the region to fill and
	 * strokes it.
	 */
	...
	/**
	 * Closes the path, fills it using the non-zero winding number rule to determine the region to
	 * fill and strokes it.
	 */
	...
	/**
	 * Closes the path, fills it using the even-odd rule to determine the region to fill and strokes
	 * it.
	 */
	...
	/**
	 * Adds an <CODE>Image</CODE> to the page. The positioning of the <CODE>Image</CODE> is done
	 * with the transformation matrix. To position an <CODE>image</CODE> at (x,y) use
	 ...
	 * Adds an <CODE>Image</CODE> to the page. The positioning of the <CODE>Image</CODE> is done
	 * with the transformation matrix. To position an <CODE>image</CODE> at (x,y) use
	 * addImage(image, image_width, 0, 0, image_height, x, y). The image can be placed inline.
	 * 
	 * @param image the <CODE>Image</CODE> object
	 * @param a an element of the transformation matrix
	 * @param b an element of the transformation matrix
	...
	/**
	 * Saves the graphic state. <CODE>saveState</CODE> and <CODE>restoreState</CODE> must be
	 * balanced.
	 */
	...
	/**
	 * Restores the graphic state. <CODE>saveState</CODE> and <CODE>restoreState</CODE> must be
	 * balanced.
	 */
	...
	/**
	 * A helper to insert into the content stream the <CODE>text</CODE> converted to bytes according
	...
	/**
	 * Moves to the next line and shows text string, using the given values of the character and
	...
	/**
	 * Computes the width of the given string taking in account the current values of "Character
	 * spacing", "Word Spacing" and "Horizontal Scaling". The additional spacing is not computed for
	 ...
	 * <P>
	 * (x1, y1) and (x2, y2) are the corners of the enclosing rectangle. Angles, measured in
	 * degrees, start with 0 to the right (the positive X axis) and increase counter-clockwise. The
	 * arc extends from startAng to startAng+extent. I.e. startAng=0 and extent=180 yields an
	 * openside-down semi-circle.
	 * <P>
	 * The resulting coordinates are of the form float[]{x1,y1,x2,y2,x3,y3, x4,y4} such that the
	 * curve goes from (x1, y1) to (x4, y4) with (x2, y2) and (x3, y3) as their respective Bezier
	 * control points.
	 * <P>
	 * Note: this code was taken from ReportLab (www.reportlab.org), an excellent PDF generator for
	 * Python (BSD license: http://www.reportlab.org/devfaq.html#1.3 ).
	 *
	...
	/**
	 * Draws a partial ellipse inscribed within the rectangle x1,y1,x2,y2, starting at startAng
	 * degrees and covering extent degrees. Angles start with 0 to the right (+x) and increase
	 * counter-clockwise.
	 *
	 * @param x1 a corner of the enclosing rectangle
	 * @param y1 a corner of the enclosing rectangle
	 ...
	 * @param xstep the desired horizontal spacing between pattern cells. May be either positive or
	 *            negative, but not zero.
	 * @param ystep the desired vertical spacing between pattern cells. May be either positive or
	 *            negative, but not zero.
	...
	/**
	 * Create a new colored tiling pattern. Variables xstep and ystep are set to the same values of
	 * width and height.
	 * 
	 * @param width the width of the pattern
	 * @param height the height of the pattern
	...
	/**
	 * Create a new uncolored tiling pattern. Variables xstep and ystep are set to the same values
	 * of width and height.
	 * 
	 * @param width the width of the pattern
	 * @param height the height of the pattern
	 ...
	 * <P>
	 * Creates a new template that is nothing more than a form XObject. This template can be
	 * included in this <CODE>PdfContentByte</CODE> or in another template. Templates are only
	 * written to the output when the document is closed permitting things like showing text in the
	 * first page that is only defined in the last page.
	 *
	 ...
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0 (no ink) and 1 (maximum
	 * ink). This method however accepts only integers between 0x00 and 0xFF.
	 * </P>
	 *
	 ...
	 * </P>
	 * Following the PDF manual, each operand must be a number between 0 (minimum intensity) and 1
	 * (maximum intensity). This method however accepts only integers between 0x00 and 0xFF.
	 *
	...
	/**
	 * Sets the stroke color. <CODE>color</CODE> can be an <CODE>ExtendedColor</CODE>.
	 * 
	...
	/**
	 * Sets the fill color. <CODE>color</CODE> can be an <CODE>ExtendedColor</CODE>.
	 * 
	 * @param color the color
	 */
	...
	/**
	 * Gets a duplicate of this <CODE>PdfContentByte</CODE>. All the members are copied by reference
	 * but the buffer stays different.
	 *
	...
	/**
	 * Gets a <CODE>Graphics2D</CODE> to write on. The graphics are translated to PDF commands as
	 * shapes. No PDF fonts will appear.
	 * 
	...
	/**
	 * Gets a <CODE>Graphics2D</CODE> to print on. The graphics are translated to PDF commands as
	 * shapes. No PDF fonts will appear.
	 * 
	 * @param width the width of the panel
	 * @param height the height of the panel
	...
	/**
	 * Begins a graphic block whose visibility is controlled by the <CODE>layer</CODE>. Blocks can
	 * be nested. Each block must be terminated by an {@link #endLayer()}.
	 * <p>
	 * Note that nested layers with {@link PdfLayer#addChild(PdfLayer)} only require a single call
	 * to this method and a single call to {@link #endLayer()}; all the nesting control is built in.
	 * 
	 ...
	 * @param obj the colorspace. A <CODE>null</CODE> or <CODE>PdfNull</CODE> removes any colorspace
	 *            with the same name
	 */
	...
	/**
	 * Begins a marked content sequence. This sequence will be tagged with the structure
	 * <CODE>struc</CODE>. The same structure can be used several times to connect text that belongs
	 * to the same logical segment but is in a different location, like the same paragraph crossing
	...
	/**
	 * Begins a marked content sequence. If property is <CODE>null</CODE> the mark will be of the
	 * type <CODE>BMC</CODE> otherwise it will be <CODE>BDC</CODE>.
	 * 
	 * @param tag the tag
	...
	/**
	 * Checks for any dangling state: Mismatched save/restore state, begin/end text, begin/end
	 * layer, or begin/end marked content sequence. If found, this function will throw. This
	 * function is called automatically during a reset() (from Document.newPage() for example), and
	 * before writing itself out in toPdf(). One possible cause: not calling
	 * myPdfGraphics2D.dispose() will leave dangling saveState() calls.
	 * 
	 * @since 2.1.6
