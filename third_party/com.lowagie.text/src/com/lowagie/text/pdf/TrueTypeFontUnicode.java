...
/**
 * Represents a True Type font with Unicode encoding. All the character in the font can be used
 * directly by using the encoding Identity-H or Identity-V. This is the only way to represent some
 * character sets such as Thai.
 * 
...
class TrueTypeFontUnicode extends TrueTypeFont implements Comparator {
	...
	/**
	 * Creates a new TrueType font addressed by Unicode characters. The font will always be
	 * embedded.
	 * 
	...
	TrueTypeFontUnicode(String ttFile, String enc, boolean emb, byte ttfAfm[], boolean forceRead) throws DocumentException, IOException {
		...
		if ((fileName.toLowerCase().endsWith(".ttf") || fileName.toLowerCase().endsWith(".otf") || fileName.toLowerCase().endsWith(".ttc")) && (enc.equals(IDENTITY_H) || enc.equals(IDENTITY_V)) && emb) {
			...
			// throw new DocumentException(fileName + " " + style + " does not contain an usable
			// cmap.");
			if (fontSpecific) {
				fontSpecific = false;
	...
	private PdfDictionary getCIDFontType2(PdfIndirectReference fontDescriptor, String subsetPrefix, Object metrics[]) {
		...
		if (!vertical) {
			...
			for (Object metric2 : metrics) {
				int metric[] = (int[]) metric2;
				if (metric[1] == 1000) {
					continue;
				}
				int m = metric[0];
