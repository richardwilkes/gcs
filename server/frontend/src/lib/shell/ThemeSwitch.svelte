...
<script lang='ts'>
	import { currentTheme, ThemeKind } from '$lib/theme.ts';
</script>

<div class='content'>
	<div class='highlight'
			 class:left={$currentTheme.current === ThemeKind.Light}
			 class:middle={$currentTheme.current === ThemeKind.System}
			 class:right={$currentTheme.current === ThemeKind.Dark} />
	<button class='button' on:click={() => $currentTheme.current = ThemeKind.Light} title='Light Mode'>
		<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' class='svg'
				 class:active={$currentTheme.current === ThemeKind.Light}>
			<path
	...
	<button class='button' on:click={() => $currentTheme.current = ThemeKind.System} title='System Mode'>
		<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 384 512' class='svg'
				 class:active={$currentTheme.current === ThemeKind.System}>
			<path
	...
	<button class='button' on:click={() => $currentTheme.current = ThemeKind.Dark} title='Dark Mode'>
		<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 384 512' class='svg'
				 class:active={$currentTheme.current === ThemeKind.Dark}>
			<path
