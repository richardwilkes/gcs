...
func (ex *legacyExporter) emitKey(key string) error {
	...
	case "BEST_CURRENT_PARRY":
		best := "-"
		bestValue := fxp.Min
		for _, w := range ex.entity.Weapons(true, false, true) {
...
// the DR printed for the location is arrived at (see HitLocation.accumulateDR and Entity.AddDRBonusesFor), so that
// the two agree:
//
...
//   - A location inherits the DR of the locations that own it, so a bonus naming an owning location -- the torso, for
//     a location in the torso's sub-table -- covers the location as well. A bonus naming the "all" location covers
//     every location, since the topmost owning location is always a top-level one and "all" applies there.
...
func (ex *legacyExporter) hitLocationEquipment(location *HitLocation) []string {
	ids := locationAndOwnerIDs(location)
	var list []string
	Traverse(func(eqp *Equipment) bool {
		if !eqp.ReallyEquipped() {
			return false
		}
		grantsDR := drBonusCoversAnyLocation(eqp.Features, eqp.SwitchedOn, ids) ||
			anyEnabledNonContainerModifier(eqp.Modifiers, func(mod *EquipmentModifier) bool {
				return drBonusCoversAnyLocation(mod.Features, eqp.SwitchedOn, ids)
			})
		if grantsDR {
...
// locationAndOwnerIDs returns the ID of the given hit location followed by the IDs of the locations that own it, from
// the nearest outward, i.e. the locations whose DR the given one inherits (see HitLocation.accumulateDR).
func locationAndOwnerIDs(location *HitLocation) []string {
	var ids []string
	for loc := location; loc != nil; {
		ids = append(ids, loc.LocID)
		if loc.owningTable == nil {
			break
		}
		loc = loc.owningTable.owningLocation
	}
	return ids
}

// drBonusCoversAnyLocation returns true if any of the given features that is active for an owner whose switch is in
// the given state is a DR bonus naming one of the given hit location IDs, either explicitly or via the "all" location.
// The switch is applied here rather than by way of Features.Active, since this runs once per hit location for every
// piece of equipment during an export and Features.Active allocates whenever it has something to filter out.
func drBonusCoversAnyLocation(features Features, switchedOn bool, ids []string) bool {
