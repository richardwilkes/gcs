...
// A spell resolved without an owning entity (as happens for library and template files) reports 0 for its level and
// relative level rather than leaking the fxp.Min sentinel that CalculateSpellLevel leaves behind.
...
// A spell with fewer than one point, which CalculateSpellLevel marks uncomputable with the fxp.Min sentinel, reports 0
// to scripts instead.
...
// A normally-resolvable spell still reports its real level and relative level, so the sentinel guard doesn't disturb
...
// A spell without a tech level hands scripts the same empty string the skill wrapper does, rather than undefined, so
// the common "no TL" idiom behaves the same for both node types.
...
// The spell wrapper reports its difficulty in the same serialization form the skill wrapper uses, so case-sensitive JS
// comparisons work across both node types.
