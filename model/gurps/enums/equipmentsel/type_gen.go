...
// Possible values.
const (
	ThisEquipment Type = iota
	EquipmentWithName
)

// DefaultType is the default value.
const DefaultType Type = ThisEquipment

// FirstType is the first valid value.
const FirstType Type = ThisEquipment

// LastType is the last valid value.
const LastType Type = EquipmentWithName
...
// Type holds the type of an equipment selection.
type Type byte
...
func (enum Type) Key() string {
	...
	case EquipmentWithName:
		return "equipment_with_name"
	default:
...
func (enum Type) String() string {
	switch enum {
	case ThisEquipment:
		return i18n.Text(`to this equipment`)
	case EquipmentWithName:
		return i18n.Text(`to equipment whose name`)
	default:
